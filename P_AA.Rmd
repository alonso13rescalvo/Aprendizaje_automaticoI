...
# Visualización: Histograma con densidad
hist(variable, main="Histograma con Densidad", xlab="Valores", col="lightblue", border="black", probability=TRUE)
lines(density(variable, na.rm=TRUE), col="red", lwd=2)  # Agregar densidad

# Pruebas de normalidad (Usar Anderson-Darling en lugar de Shapiro-Wilk debido al tamaño de los datos)
ad_test <- ad.test(variable)  # Anderson-Darling
ks_test <- ks.test(variable, "pnorm", mean(variable, na.rm=TRUE), sd(variable, na.rm=TRUE))  # Kolmogorov-Smirnov
